...
Single source of truth per project: `CLAUDE.md` (regular file), bridged to
`AGENTS.md` (symlink). Where symlinks are unreliable (Windows), fall back to
the reversed pair: `AGENTS.md` as the file plus a `CLAUDE.md` containing
`@AGENTS.md` — Codex cannot import, so AGENTS.md must be real content there.
